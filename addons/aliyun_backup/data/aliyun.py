...
if __name__ == '__main__':
    ...
    user = ali.get_user()  # 
    print(user.user_name, user.nick_name, user.phone,user.default_drive_id)  # 
    ...
    for file in file_list:  # 
        ...
        if file.type == 'folder':
            #print(file.name)
            if file.name == 'IT Commander HA Backup':
                FolderID = file.file_id
                CreateFolder = False
                print(file.name, file.file_id)

    if CreateFolder == True :
        result = ali.create_folder(name='IT Commander HA Backup')
        FolderID = result.file_id
        print(result.file_name,result.file_id)
    
    result = ali.upload_files(file_paths=['test1.txt'], parent_file_id=FolderID, drive_id=user.default_drive_id)
    print(result)
